...
Given an N, get some k' value
k' is for everything to be on
Thus, ON iff K == k' + (1 + k') * m, where m is a positive integer

To generate k' given an N:
A0 = 0;
A1 = 1;
An = 2 * A(n-1) + 1
...
vector<int> MakeNToKPrime(int limit)
{
    vector<int> nToKPrime(limit + 1);

    nToKPrime[0] = 0;

    for (int i=1; i<=limit; ++i)
    {
        nToKPrime[i] = nToKPrime[i-1] * 2 + 1;
    }
...
{
    static vector<int> nToKPrime = MakeNToKPrime(30);

    int kPrime = nToKPrime[N];
